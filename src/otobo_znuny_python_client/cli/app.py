...
from otobo_znuny_python_client.cli.interface import OtoboCommandRunner, OtoboConsole, Permission
...
def _build_runner(console_path: Path, docker_container: Optional[str]) -> OtoboCommandRunner:
    if docker_container:
        return OtoboCommandRunner.from_docker(container=docker_container, console_path=str(console_path))
    return OtoboCommandRunner.from_local(console_path=str(console_path))


def _build_console(console_path: Path, docker_container: Optional[str]) -> OtoboConsole:
    return OtoboConsole(_build_runner(console_path, docker_container))
...
) -> SystemEnvironment | DockerEnvironment:
    if docker_container:
        return DockerEnvironment(docker_container, str(console_path), webservices_dir)
    return SystemEnvironment(console_path, webservices_dir)
...
def add_user(
        ...
        console_path: Path = typer.Option(Path("/opt/otobo/bin/otobo.Console.pl"), help="Path to the OTOBO console executable."),
        docker_container: Optional[str] = typer.Option(None, help="Docker container name if the console should be executed inside a container."),
) -> None:
    console = _build_console(console_path, docker_container)
...
def interactive_setup(
        console_path: Path = typer.Option(Path("/opt/otobo/bin/otobo.Console.pl"), help="Path to the OTOBO console executable."),
        webservices_dir: Path = typer.Option(Path("/opt/otobo/var/webservices"), help="Directory for generated webservice definitions."),
        docker_container: Optional[str] = typer.Option(None, help="Docker container name if the console should be executed inside a container."),
) -> None:
    env = _build_environment(console_path, webservices_dir, docker_container)
